...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabaseAdmin
    .from('products')
    .select('id, name, brand, brl_price, brl_price_promo, usd_price, usd_price_promo, img_url, imagens, estoque, categoria_id, descricao, descricao_curta, badges, published_at, multiplicador, venda_minima, unidade_venda, custom_fields, sku, limite_por_cpf')
    .eq('id', id)
    .eq('ativo', true)
