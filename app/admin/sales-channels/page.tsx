          ...
          <tbody>
            {loading ? (
              ...
              <tr key={c.id} style={{ borderBottom: '1px solid var(--a-border)' }}>
                <td style={{ padding: '12px 18px', fontSize: 12, color: '#8b5cf6', fontFamily: 'monospace', fontWeight: 700 }}>{c.slug}</td>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--a-surface)', border: '1px solid #2a2a2a', borderRadius: 16, width: '100%', maxWidth: 440, padding: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <button onClick={criar} disabled={saving || !form.slug || !form.nome}
                style={{ marginTop: 8, padding: '13px', background: (!form.slug || !form.nome) ? 'var(--a-border)' : '#8b5cf6', color: (!form.slug || !form.nome) ? 'var(--a-text3)' : '#000', border: 'none', borderRadius: 10, fontWeight: 900, fontSize: 14, cursor: saving ? 'wait' : 'pointer' }}>
